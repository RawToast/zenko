...
describe("collectInlineTypes", () => {
  ...
  describe("collectInlineErrorTypes", () => {
    test("should collect inline error response schemas", () => {
      const operations = [mockOperation]
      const spec = {
        paths: {
          "/test": {
      ...
      const result = collectInlineErrorTypes(operations, spec)

      expect(result.size).toBe(1)
      expect(result.has("TestOperationBadRequest")).toBe(true)
      expect(result.get("TestOperationBadRequest")).toEqual({
        type: "object",
        properties: {
          error: { type: "string" },
        },
    ...
    test("should collect error schemas with allOf", () => {
      const operations = [mockOperation]
      const spec = {
        paths: {
          "/test": {
      ...
      expect(result.size).toBe(1)
      expect(result.has("TestOperationNotFound")).toBe(true)
    })

    test("should collect error schemas with oneOf", () => {
      const operations = [mockOperation]
      const spec = {
        paths: {
          "/test": {
      ...
      expect(result.size).toBe(1)
      expect(result.has("TestOperationUnprocessableEntity")).toBe(true)
    })

    test("should collect error schemas with anyOf", () => {
      const operations = [mockOperation]
      const spec = {
        paths: {
          "/test": {
      ...
      expect(result.size).toBe(1)
      expect(result.has("TestOperationInternalServerError")).toBe(true)
    })

    test("should not collect simple $ref error schemas", () => {
      const operations = [mockOperation]
      const spec = {
        paths: {
          "/test": {
    ...
    test("should only collect error status codes (4xx, 5xx, default)", () => {
      const operations = [mockOperation]
      const spec = {
        paths: {
          "/test": {
      ...
      expect(result.has("TestOperationNotFound")).toBe(true)
      expect(result.has("TestOperationInternalServerError")).toBe(true)
      expect(result.has("TestOperationDefaultError")).toBe(true)
      // Should not collect 200 response
      expect(result.has("TestOperationResponse")).toBe(false)
    })

    test("should not collect error schemas without application/json content", () => {
      const operations = [mockOperation]
      const spec = {
        paths: {
          "/test": {
    ...
    test("should not collect error schemas without content", () => {
      const operations = [mockOperation]
      const spec = {
        paths: {
          "/test": {
    ...
    test("should handle webhook error schemas", () => {
      const operations = [mockOperation]
      const spec = {
        webhooks: {
          "webhook-name": {
    ...
    test("should handle hyphenated operation IDs", () => {
      ...
      expect(result.size).toBe(1)
      expect(result.has("TestOperationWithHyphensNotFound")).toBe(true)
    })
    ...
    test("should handle multiple error status codes for same operation", () => {
      const operations = [mockOperation]
      const spec = {
        paths: {
          "/test": {
  ...
  describe("combined behavior", () => {
    ...
    test("should collect request, response, and error types from the same operation", () => {
      const operations = [mockOperation]
      const spec = {
        paths: {
          "/test": {
      ...
      const requestTypes = collectInlineRequestTypes(operations, spec)
      const responseTypes = collectInlineResponseTypes(operations, spec)
      const errorTypes = collectInlineErrorTypes(operations, spec)
      ...
      expect(errorTypes.size).toBe(2)
      expect(errorTypes.has("TestOperationBadRequest")).toBe(true)
      expect(errorTypes.has("TestOperationNotFound")).toBe(true)
    })
  })
})
